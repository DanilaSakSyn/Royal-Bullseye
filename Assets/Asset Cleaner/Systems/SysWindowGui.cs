...
namespace Asset_Cleaner {
    ...
    class SysWindowGui : IEcsRunSystem, IEcsInitSystem {
        ...
        void ShowTabSettings(Config conf) {
            using (new EditorGUILayout.VerticalScope()) {
                ...
                using (new EditorGUILayout.VerticalScope()) {
                    ...
                    conf.IgnoreMonoScript = GUILayout.Toggle(conf.IgnoreMonoScript, "Skip MonoScript");
        ...
        static bool IsScript (string filename) {
            return filename.EndsWith (".cs", StringComparison.OrdinalIgnoreCase) || filename.EndsWith (".cs.meta", StringComparison.OrdinalIgnoreCase);
        }
        ...
        void RenderRows(WindowData windowData) {
            ...
            if (windowData.ExpandScenes && windowData.FindFrom == FindModeEnum.Scene) {
                foreach (var (grp, indices) in SceneResultRows.Out(out _, out var get2, out _, out _).GroupBy1(ResultComp.Instance)) {
